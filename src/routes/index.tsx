...
import { Reveal } from "@/components/site/HomeShowcase";
import { StudentResults } from "@/components/site/StudentResults";
import { createFileRoute, Link } from "@tanstack/react-router";
import { ArrowDownRight, ArrowRight, BookOpen, Check, FileCheck2, GraduationCap, Laptop2, MessageCircle, Play, Quote, Target, Trophy } from "lucide-react";
import { TEACHER_PORTRAIT } from "@/lib/site-data";
import { LmsButton } from "@/components/site/LmsButton";
import { CLASSES, FAQS, LMS_URL, TEACHER, WHATSAPP_URL } from "@/lib/site-data";
import { TimetableShowcase } from "@/components/site/TimetableShowcase";
import { Accordion, AccordionContent, AccordionItem, AccordionTrigger } from "@/components/ui/accordion";
import lmsMarketplacePreview from "@/assets/course-marketplace-preview.png";
...
function Home() {
  return <>
    <section className="hero-stage">
      ...
      <div className="section-x hero-inner">
        <div className="hero-copy">
          ...
          <div className="hero-actions"><Link to="/classes" className="button-electric">Explore Classes <ArrowRight className="size-4" /></Link><LmsButton size="lg" className="button-ghost-light">Student LMS <ArrowDownRight className="size-4" /></LmsButton><a href={WHATSAPP_URL} target="_blank" rel="noopener noreferrer" className="button-ghost-light hero-whatsapp"><MessageCircle className="size-4" /> WhatsApp</a></div>
    ...
    <section className="final-cta" id="contact"><div className="section-x"><Quote className="cta-quote"/><p className="si">ඔබේ result එක වෙනස් කරන<br/>decision එක අද ගන්න.</p><h2>Start before you feel ready.</h2><div><Link to="/classes" className="button-electric">Join a class <ArrowRight className="size-4"/></Link><a href={WHATSAPP_URL} target="_blank" rel="noopener noreferrer" className="text-link-light"><MessageCircle className="size-4" /> Chat on WhatsApp</a><a href={LMS_URL} className="text-link-light">Already a student? Login</a></div></div></section>
  </>;
}
